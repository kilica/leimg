...
define('LEIMG_FRAME_PERFORM_SUCCESS', 1);
define('LEIMG_FRAME_PERFORM_FAIL', 2);
define('LEIMG_FRAME_INIT_SUCCESS', 3);

define('LEIMG_FRAME_VIEW_NONE','none');
define('LEIMG_FRAME_VIEW_SUCCESS','success');
define('LEIMG_FRAME_VIEW_ERROR','error');
define('LEIMG_FRAME_VIEW_INDEX','index');
define('LEIMG_FRAME_VIEW_INPUT','input');
define('LEIMG_FRAME_VIEW_PREVIEW','preview');
define('LEIMG_FRAME_VIEW_CANCEL','cancel');

/**
 * Leimg_Module
**/
class Leimg_Module extends Legacy_ModuleAdapter
{
	public /*** string ***/ $mActionName = null;

	public /*** Leimg_AbstractAction ***/ $mAction = null;

	public /*** bool ***/ $mAdminFlag = false;

	public /*** Leimg_AssetManager ***/ $mAssetManager = null;
	...
	protected /*** Enum[] ***/ $_mAllowViewNames = array(
		LEIMG_FRAME_VIEW_NONE,
		LEIMG_FRAME_VIEW_SUCCESS,
		LEIMG_FRAME_VIEW_ERROR,
		LEIMG_FRAME_VIEW_INDEX,
		LEIMG_FRAME_VIEW_INPUT,
		LEIMG_FRAME_VIEW_PREVIEW,
		LEIMG_FRAME_VIEW_CANCEL
	);
	...
	**/
	public function startup()
	{
		...
		XCube_DelegateUtils::call('Module.leimg.Global.Event.GetAssetManager',new XCube_Ref($this->mAssetManager),$this->mXoopsModule->get('dirname'));
	...
	**/
	private function _getDefaultActionName()
	{
		...
		$dataname = isset($dataname) ? $dataname : 'Thumbnail';
	...
	**/
	public function getRenderSystemName()
	{
		...
		if(!defined('LEIMG_ADMIN_RENDER_REGISTED'))
		{
			define('LEIMG_ADMIN_RENDER_REGISTED',true);
			$root =& XCube_Root::getSingleton();
			$root->overrideSiteConfig(
				array(
					'RenderSystems' => array(
						'Leimg_AdminRenderSystem' => 'Leimg_AdminRenderSystem'
					),
					'Leimg_AdminRenderSystem' => array(
						'root' => LEIMG_TRUST_PATH,
						'path' => '/admin/class/LeimgAdminRenderSystem.class.php',
						'class' => 'Leimg_AdminRenderSystem'
					)
				)
			);
		}
	
		return 'Leimg_AdminRenderSystem';
	}
	...
	**/
	public function execute(/*** XCube_Controller ***/ &$controller)
	{
		...
		$viewStatus = (Leimg_Utils::getEnv('REQUEST_METHOD') == 'POST') ?
	...
	private function _createAction()
	{
		...
		switch(true)
		{
			case file_exists(
				...
				break;
			case file_exists(
				$path = LEIMG_TRUST_PATH . '/' . $fileName
			):
				break;
			default:
				return false;
		...
		$className = 'Leimg_' . ($this->mAdminFlag ? 'Admin_' : '')
			. ucfirst($this->mActionName) . 'Action';
		if(class_exists($className))
		{
			$this->mAction = new $className();
		}
		if(!$this->mAction instanceof Leimg_AbstractAction)
		{
			return false;
		}
	...
	**/
	...
	{
		/**
		 * Module.leimg.Global.Event.Exception.ActionNotFound
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.leimg.Global.Event.Exception.ActionNotFound',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		/**
		 * Module.leimg.Global.Event.Exception.Preparation
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.leimg.Global.Event.Exception.Preparation',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		/**
		 * Module.leimg.Global.Event.Exception.Permission
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.leimg.Global.Event.Exception.Permission',$this->mAssetManager->mDirname);
		/**
