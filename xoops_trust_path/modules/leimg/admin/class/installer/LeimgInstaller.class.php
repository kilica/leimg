...
class Leimg_Installer
{
    ...
    **/
    ...
    {
        return Leimg_InstallUtils::installSQLAutomatically(
            $this->_mXoopsModule,
            $this->mLog
        );
    }
    ...
    **/
    private function _installModule()
    {
        ...
        {
            $this->mLog->addError(_MI_LEIMG_INSTALL_ERROR_MODULE_INSTALLED);
            return false;
        }
        ...
        if($this->_mXoopsModule->getInfo('hasAdmin'))
        {
            ...
            {
                $this->mLog->addError(_MI_LEIMG_INSTALL_ERROR_PERM_ADMIN_SET);
            }
        }
        ...
        if($this->_mXoopsModule->getInfo('hasMain'))
        {
            if($this->_mXoopsModule->getInfo('read_any'))
            {
                ...
                foreach($groupObjects as $group)
                {
                    ...
                    {
                        $this->mLog->addError(_MI_LEIMG_INSTALL_ERROR_PERM_READ_SET);
                    }
                }
            }
            else
    ...
    **/
    ...
    {
        Leimg_InstallUtils::installAllOfModuleTemplates(
            $this->_mXoopsModule,
            $this->mLog
        );
    }
    ...
    **/
    ...
    {
        Leimg_InstallUtils::installAllOfBlocks(
            $this->_mXoopsModule,
            $this->mLog
        );
    }
    ...
    **/
    ...
    {
        Leimg_InstallUtils::installAllOfConfigs(
            $this->_mXoopsModule,
            $this->mLog
        );
    }
    ...
    **/
    private function _processReport()
    {
        if(!$this->mLog->hasError())
        {
            $this->mLog->add(
                XCube_Utils::formatString(
                    _MI_LEIMG_INSTALL_MSG_MODULE_INSTALLED,
        ...
        else if(is_object($this->_mXoopsModule))
        {
            $this->mLog->addError(
                XCube_Utils::formatString(
                    _MI_LEIMG_INSTALL_ERROR_MODULE_INSTALLED,
                    $this->_mXoopsModule->getInfo('name')
                )
            );
        }
