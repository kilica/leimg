...
require_once LEIMG_TRUST_PATH . '/admin/class/installer/LeimgInstallUtils.class.php';

/**
 * Leimg_Uninstaller
**/
class Leimg_Uninstaller
{
    ...
    **/
    private function _uninstallModule()
    {
        ...
        {
            $this->mLog->addReport(_MI_LEIMG_INSTALL_MSG_MODULE_INFORMATION_DELETED);
        }
        else
        {
            $this->mLog->addError(_MI_LEIMG_INSTALL_ERROR_MODULE_INFORMATION_DELETED);
        }
    }
        ...
        if(is_array($tables))
        {
            foreach($tables as $table)
            {
                ...
                if($db->query($sql))
                {
                    $this->mLog->addReport(
                        XCube_Utils::formatString(
                            _MI_LEIMG_INSTALL_MSG_TABLE_DOROPPED,
                            $tableName
                        )
                    );
                }
                else
                {
                    $this->mLog->addError(
                        XCube_Utils::formatString(
                            _MI_LEIMG_INSTALL_ERROR_TABLE_DOROPPED,
                            $tableName
                        )
                    );
                }
    ...
    **/
    ...
    {
        Leimg_InstallUtils::uninstallAllOfModuleTemplates($this->_mXoopsModule,$this->mLog,false);
    }
    ...
    **/
    ...
    {
        Leimg_InstallUtils::uninstallAllOfBlocks($this->_mXoopsModule,$this->mLog);
        ...
        if(!$tplHandler->deleteAll($cri))
        {
            $this->mLog->addError(
                XCube_Utils::formatString(
                    _MI_LEIMG_INSTALL_ERROR_BLOCK_TPL_DELETED,
                    $tplHandler->db->error()
                )
            );
        }
    ...
    **/
    ...
    {
        Leimg_InstallUtils::uninstallAllOfConfigs($this->_mXoopsModule,$this->mLog);
    }
    ...
    **/
    private function _processReport()
    {
        if(!$this->mLog->hasError())
        {
            $this->mLog->add(
                XCube_Utils::formatString(
                    _MI_LEIMG_INSTALL_MSG_MODULE_UNINSTALLED,
        ...
        else if(is_object($this->_mXoopsModule))
        {
            $this->mLog->addError(
                XCube_Utils::formatString(
                    _MI_LEIMG_INSTALL_ERROR_MODULE_UNINSTALLED,
                    $this->_mXoopsModule->get('name')
                )
            );
        }
