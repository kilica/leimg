...
if(class_exists('Leimg_InstallUtils'))
{
    return;
}
...
class Leimg_InstallUtils
{
        ...
        if(!$scanner->loadFile($sqlFilePath))
        {
            $log->addError(
                XCube_Utils::formatString(
                    _MI_LEIMG_INSTALL_ERROR_SQL_FILE_NOT_FOUND,
                    $sqlFile
                )
            );
            return false;
    ...
    public static function DBquery(/*** string ***/ $query,/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        foreach($sqls as $sql)
        {
            if($root->mController->mDB->query($sql))
            {
                $log->addReport(
                    XCube_Utils::formatString(
                        _MI_LEIMG_INSTALL_MSG_SQL_SUCCESS,
                        $sql
                    )
                );
            }
            else
            {
                $log->addReport(
                    XCube_Utils::formatString(
                        _MI_LEIMG_INSTALL_MSG_SQL_ERROR,
                        $sql
                    )
                );
                $successFlag = false;
    ...
    **/
    public static function installAllOfModuleTemplates(/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        if(is_array($templates) && count($templates) > 0)
        {
            ...
            {
                Leimg_InstallUtils::installModuleTemplate($module,$template,$log);
            }
        }
    }
    ...
    **/
    public static function installModuleTemplate(/*** XoopsModule ***/ &$module,/*** string[] ***/ $template,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        $filename   =  Leimg_InstallUtils::replaceDirname(trim($template['file']),$dirname,$trustDirname);
        $tplData    =  Leimg_InstallUtils::readTemplateFile($dirname,$trustDirname,$filename['trust']);
        ...
        if($tplHandler->insert($tplFile))
        {
            $log->addReport(
                XCube_Utils::formatString(
                    _MI_LEIMG_INSTALL_MSG_TPL_INSTALLED,
                    $filename['public']
                )
            );
        }
        else
        {
            $log->addError(
                XCube_Utils::formatString(
                    _MI_LEIMG_INSTALL_ERROR_TPL_INSTALLED,
                    $filename['public']
                )
            );
            return false;
        ...
        if(is_array($delTemplates) && count($delTemplates) > 0)
        {
            ...
            foreach($delTemplates as $tpl)
            {
                if(!$tplHandler->delete($tpl))
                {
                    $log->addError(
                        XCube_Utils::formatString(
                            _MI_LEIMG_INSTALL_ERROR_TPL_UNINSTALLED,
                            $tpl->get('tpl_file')
                        )
                    );
                }
    ...
    **/
    public static function installAllOfBlocks(/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        if(is_array($blocks) && count($blocks) > 0)
        {
            ...
            {
                $newBlock =& Leimg_InstallUtils::createBlockByInfo($module,$block);
                Leimg_InstallUtils::installBlock($module,$newBlock,$block,$log);
            }
        }
        return true;
    }
    ...
    **/
    public static function &createBlockByInfo(/*** XoopsModule ***/ &$module,/*** string[] ***/ $block)
    {
        ...
        $filename = isset($block['template']) ?
            Leimg_InstallUtils::replaceDirname($block['template'],$module->get('dirname')) :
            null;
    
        $blockHandler =& Leimg_Utils::getXoopsHandler('block');
    ...
    **/
    public static function installBlock(/*** XoopsModule ***/ &$module,/*** XoopsBlock ***/ &$blockObj,/*** string[] ***/ &$block,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        if(!$blockHandler->insert($blockObj,$autoLink))
        {
            $log->addError(
                XCube_Utils::formatString(
                    _MI_LEIMG_INSTALL_ERROR_BLOCK_INSTALLED,
        ...
        $log->addReport(
            XCube_Utils::formatString(
                _MI_LEIMG_INSTALL_MSG_BLOCK_INSTALLED,
                $blockObj->getVar('name')
            )
        );
    
        Leimg_InstallUtils::installBlockTemplate($blockObj,$module,$log);
        ...
        if($autoLink)
        {
            ...
            if(!$blockHandler->db->query($sql))
            {
                $log->addWarning(
                    XCube_Utils::formatString(
                        _MI_LEIMG_INSTALL_ERROR_BLOCK_COULD_NOT_LINK,
                        $blockObj->getVar('name')
                    )
                );
            }
        ...
        $gpermHandler =& Leimg_Utils::getXoopsHandler('groupperm');
        ...
        {
            $memberHandler =& Leimg_Utils::getXoopsHandler('member');
            ...
            foreach($groups as $group)
            {
                ...
                if(!$gpermHandler->insert($perm))
                {
                    $log->addWarning(
                        XCube_Utils::formatString(
                            _MI_LEIMG_INSTALL_ERROR_PERM_COULD_NOT_SET,
                            $blockObj->getVar('name')
                        )
                    );
                }
        ...
        else
        {
            foreach(array(XOOPS_GROUP_ADMIN,XOOPS_GROUP_USERS) as $group)
            {
                ...
                if(!$gpermHandler->insert($perm))
                {
                    $log->addWarning(
                        XCube_Utils::formatString(
                            _MI_LEIMG_INSTALL_ERROR_BLOCK_PERM_SET,
                            $blockObj->getVar('name')
                        )
                    );
                }
    ...
    **/
    public static function installBlockTemplate(/*** XoopsBlock ***/ &$block,/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        $filename = Leimg_InstallUtils::replaceDirname(
        ...
        $tplSource = Leimg_InstallUtils::readTemplateFile(
            $module->get('dirname'),
            $module->getInfo('trust_dirname'),
            $filename['trust'],
            true
        ...
        if($tplHandler->insert($tplFile))
        {
            $log->addReport(
                XCube_Utils::formatString(
                    _MI_LEIMG_INSTALL_MSG_BLOCK_TPL_INSTALLED,
                    $filename['public']
                )
            );
            return true;
        ...
        $log->addError(
            XCube_Utils::formatString(
                _MI_LEIMG_INSTALL_ERROR_BLOCK_TPL_INSTALLED,
                $filename['public']
            )
        );
        return false;
    ...
    public static function uninstallAllOfBlocks(/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        foreach($blocks as $block)
        {
            if($blockHandler->delete($block))
            {
                $log->addReport(
                    XCube_Utils::formatString(
                        _MI_LEIMG_INSTALL_MSG_BLOCK_UNINSTALLED,
                        $block->get('name')
                    )
                );
            }
            else
            {
                $log->addWarning(
                    XCube_Utils::formatString(
                        _MI_LEIMG_INSTALL_ERROR_BLOCK_UNINSTALLED,
                        $block->get('name')
                    )
                );
                $successFlag = false;
            ...
            if(!$gpermHandler->deleteAll($cri))
            {
                $log->addWarning(
                    XCube_Utils::formatString(
                        _MI_LEIMG_INSTALL_ERROR_BLOCK_PERM_DELETE,
                        $block->get('name')
                    )
                );
                $successFlag = false;
    ...
    **/
    ...
    {
        $blockHandler =& Leimg_Utils::getModuleHandler('newblocks','legacy');
        $cri = new CriteriaCompo();
        ...
        foreach($blocks as $block)
        {
            Leimg_InstallUtils::uninstallBlockTemplate($block,$module,$log,true);
            Leimg_InstallUtils::installBlockTemplate($block,$module,$log);
        }
    }
    ...
    public static function updateBlockByInfo(/*** Legacy_BlockInformation ***/ &$info,/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        foreach($blocks as $block)
        {
            ...
            if($blockHandler->insert($block))
            {
                $log->addReport(
                    XCube_Utils::formatString(
                        _MI_LEIMG_INSTALL_MSG_BLOCK_UPDATED,
                        $block->get('name')
                    )
                );
            }
            else
            {
                $log->addError(
                    XCube_Utils::formatString(
                        _MI_LEIMG_INSTALL_ERROR_BLOCK_UPDATED,
                        $block->get('name')
                    )
                );
            }
    ...
    **/
    public static function uninstallBlockTemplate(/*** XoopsBlock ***/ &$block,/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log,/*** bool ***/ $defaultOnly = false)
    {
        ...
        $log->addReport(
            XCube_Utils::formatString(
                _MI_LEIMG_INSTALL_MSG_BLOCK_TPL_UNINSTALLED,
                $block->get('template')
            )
        );
        return true;
    ...
    public static function installAllOfConfigs(/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        foreach($preferences->mPreferences as $info)
        {
            ...
            if($configHandler->insertConfig($config))
            {
                $log->addReport(
                    XCube_Utils::formatString(
                        _MI_LEIMG_INSTALL_MSG_CONFIG_ADDED,
            ...
            else
            {
                $log->addError(
                    XCube_Utils::formatString(
                        _MI_LEIMG_INSTALL_ERROR_CONFIG_ADDED,
                        $config->get('conf_name')
                    )
                );
                $successFlag = false;
    ...
    public static function uninstallAllOfConfigs(/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        foreach($configs as $config)
        {
            if($configHandler->deleteConfig($config))
            {
                $log->addReport(
                    XCube_Utils::formatString(
                        _MI_LEIMG_INSTALL_MSG_CONFIG_DELETED,
            ...
            else
            {
                $log->addWarning(
                    XCube_Utils::formatString(
                        _MI_LEIMG_INSTALL_ERROR_CONFIG_DELETED,
                        $config->getVar('conf_name')
                    )
                );
                $sucessFlag = false;
    ...
    **/
    public static function updateConfigByInfo(/*** Legacy_PreferenceInformation ***/ &$info,/*** XoopsModule ***/ &$module,/*** Legacy_ModuleInstallLog ***/ &$log)
    {
        ...
        {
            $log->addError(_MILEIMG_INSTALL_ERROR_CONFIG_NOT_FOUND);
            return false;
        }
        ...
        if($configHandler->insertConfig($config))
        {
            $log->addReport(
                XCube_Utils::formatString(
                    _MI_LEIMG_INSTALL_MSG_CONFIG_UPDATED,
                    $config->get('conf_name')
                )
            );
            return true;
        ...
        $log->addError(
            XCube_Utils::formatString(
                _MI_LEIMG_INSTALL_ERROR_CONFIG_UPDATED,
                $config->get('conf_name')
            )
        );
        return false;
